"""Tests for team baserunners analytics.

Every calculation is checked against a hand-computed expectation rather than
against a reimplementation of the same loop. ``season_with`` isolates the
walk total as the only nonzero baserunner component, so the point values in
most tests can be read directly against the input list.
"""
...
def season_with(baserunners: list[int | None], **kwargs: object) -> list:
    """Build a team-season whose baserunners equal the given values exactly.
...
def test_points_follow_season_order() -> None:
    analysis = build_team_baserunners_analysis(
        season_with([10, 8, 12]), rolling_window=3
    )
    assert [point.season_game_number for point in analysis.points] == [1, 2, 3]
    assert [point.baserunners for point in analysis.points] == [10, 8, 12]


def test_baserunners_sum_hits_walks_and_hit_by_pitch() -> None:
    games = make_season(
        hits=[6, 2],
        base_on_balls=[5, 3],
        hit_by_pitch=[0, 1],
    )
    analysis = build_team_baserunners_analysis(games, rolling_window=2)
    assert [point.baserunners for point in analysis.points] == [11, 6]


def test_unordered_input_is_sorted_before_analysis() -> None:
    games = season_with([10, 8, 12])
    analysis = build_team_baserunners_analysis(list(reversed(games)), rolling_window=3)
    assert [point.baserunners for point in analysis.points] == [10, 8, 12]
...
def test_doubleheader_games_keep_their_real_sequence() -> None:
    ...
    analysis = build_team_baserunners_analysis(games, rolling_window=2)
    assert [point.baserunners for point in analysis.points] == [11, 4]
...
def test_first_game_is_its_own_rolling_average() -> None:
    analysis = build_team_baserunners_analysis(season_with([13, 2]), rolling_window=10)
...
def test_precision_is_kept_internally() -> None:
    ...
    analysis = build_team_baserunners_analysis(season_with([1, 1, 0]), rolling_window=3)
...
def test_prior_window_is_none_without_two_complete_windows() -> None:
    analysis = build_team_baserunners_analysis(season_with([5] * 5), rolling_window=3)
...
def test_summary_games_played_counts_stored_games() -> None:
    analysis = build_team_baserunners_analysis(season_with([4] * 7), rolling_window=5)
    assert analysis.summary.games_played == 7
    assert len(analysis.points) == 7


def test_team_and_season_come_from_the_records() -> None:
    analysis = build_team_baserunners_analysis(season_with([6, 6]), rolling_window=2)
...
def test_last_game_date_is_the_most_recent_game() -> None:
    analysis = build_team_baserunners_analysis(season_with([1, 2, 3]), rolling_window=2)
    assert analysis.last_game_date == date(2025, 3, 29)


def test_a_missing_walk_total_is_refused() -> None:
    with pytest.raises(MissingBaserunnerDataError) as excinfo:
        build_team_baserunners_analysis(season_with([10, None, 12]), rolling_window=3)
    assert excinfo.value.games_missing == 1
    assert excinfo.value.games_total == 3


def test_a_missing_hit_by_pitch_total_is_also_refused() -> None:
    """Either missing component makes the game's baserunners unknown."""
    games = make_season(
        hits=[0, 0],
        base_on_balls=[5, 3],
        hit_by_pitch=[0, None],
    ...
    with pytest.raises(MissingBaserunnerDataError) as excinfo:
        build_team_baserunners_analysis(games, rolling_window=2)
    assert excinfo.value.games_missing == 1


def test_missing_totals_are_never_read_as_zero() -> None:
    """A zero would drag the average down and look like a real result."""
    with pytest.raises(MissingBaserunnerDataError):
        build_team_baserunners_analysis(season_with([10, None]), rolling_window=2)


def test_a_fully_legacy_season_reports_every_game_as_missing() -> None:
    with pytest.raises(MissingBaserunnerDataError) as excinfo:
        build_team_baserunners_analysis(season_with([None] * 4), rolling_window=2)
...
def test_missing_data_error_names_the_re_import_remedy() -> None:
    with pytest.raises(MissingBaserunnerDataError) as excinfo:
        build_team_baserunners_analysis(season_with([None, 5]), rolling_window=2)
    assert "re-import" in str(excinfo.value)


def test_zero_baserunners_is_analysed_rather_than_refused() -> None:
    analysis = build_team_baserunners_analysis(season_with([0, 0]), rolling_window=2)
    assert analysis.summary.season_average == 0.0


def test_empty_input_is_rejected() -> None:
    with pytest.raises(TeamBaserunnersAnalysisError):
        build_team_baserunners_analysis([], rolling_window=5)


@pytest.mark.parametrize("window", [0, -1])
def test_non_positive_windows_are_rejected(window: int) -> None:
    with pytest.raises(TeamBaserunnersAnalysisError):
        build_team_baserunners_analysis(season_with([5, 5]), rolling_window=window)
...
def test_mixed_teams_are_rejected() -> None:
    ...
    with pytest.raises(TeamBaserunnersAnalysisError) as excinfo:
        build_team_baserunners_analysis(games, rolling_window=2)
...
def test_mixed_teams_are_rejected_before_missing_data_is_reported() -> None:
    ...
    with pytest.raises(TeamBaserunnersAnalysisError) as excinfo:
        build_team_baserunners_analysis(games, rolling_window=2)
    assert not isinstance(excinfo.value, MissingBaserunnerDataError)
...
def test_a_long_season_stays_consistent_end_to_end() -> None:
    ...
    analysis = build_team_baserunners_analysis(games, rolling_window=15)

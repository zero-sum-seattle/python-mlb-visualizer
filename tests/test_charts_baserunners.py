"""Tests for the baserunners figure contract, not for Plotly itself."""

import pytest

from app.analytics.league_baserunners import compare_team_baserunners_to_league
...
from app.web.charts import (
    ...
    BASERUNNERS_Y_AXIS_TITLE,
    MLB_AVERAGE_TRACE_NAME,
    RAW_BASERUNNERS_TRACE_NAME,
...
from tests.factories import make_league_baserunners_context, make_season

VALUES = [10, 4, 14, 6, 11]


def analysis_for(baserunners: list[int], window: int = 5):
...
@pytest.fixture
def figure():
    return build_team_baserunners_figure(analysis_for(VALUES))
...
def league_figure():
    ...
    league = make_league_baserunners_context(total_baserunners=82, team_game_records=10)
    return build_team_baserunners_figure(
        analysis, compare_team_baserunners_to_league(analysis, league)
    )
...
def test_only_the_mlb_line_is_labelled_when_it_is_drawn(league_figure) -> None:
    """Both lines can sit a tenth of a baserunner apart, where labels collide."""
...
def test_season_average_trace_reads_the_authoritative_summary_value() -> None:
    analysis = analysis_for(VALUES)
    figure = build_team_baserunners_figure(analysis)
    assert figure.data[2].y[0] == pytest.approx(analysis.summary.season_average)


def test_axis_titles_name_baserunners(figure) -> None:
    assert figure.layout.xaxis.title.text == X_AXIS_TITLE
    assert figure.layout.yaxis.title.text == BASERUNNERS_Y_AXIS_TITLE
...
def test_y_axis_grows_with_an_unusually_high_game() -> None:
    figure = build_team_baserunners_figure(analysis_for([2, 2, 24]))
    assert figure.layout.yaxis.range is None


def test_y_axis_starts_at_zero() -> None:
    assert (
        build_team_baserunners_figure(analysis_for(VALUES)).layout.yaxis.rangemode
        == "tozero"
    )


def test_hover_shows_date_matchup_baserunners_and_average(figure) -> None:
    template = figure.data[0].hovertemplate
    assert "Baserunners: %{customdata[2]}" in template
...
def test_rendered_html_uses_the_baserunners_div_id(figure) -> None:
    html = render_figure_html(figure, div_id=BASERUNNERS_CHART_DIV_ID)
    assert BASERUNNERS_CHART_DIV_ID in html

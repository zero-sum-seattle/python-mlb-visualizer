"""Tests for how the strikeouts page gates MLB batting strikeout context.
...
Two things must both hold before an MLB batting K/Game is shown: complete
league-season coverage, and a known batting strikeout total on every stored
record for the season. The second rule has no equivalent on the hits page,
because hits were never missing.
"""
...
from app.web.formatting import LEAGUE_STRIKEOUTS_UNAVAILABLE_NOTE
...
def seed_two_teams(seed: SeedFn, *, season: int = 2025) -> None:
    """Mariners strike out 9 times a game, Twins 7; MLB across both is 8.00."""
...
def strikeouts_page(
    ...
    *,
    ...
    url = f"/strikeouts?team_id={team_id}&season={season}"
...
def test_complete_coverage_shows_the_mlb_comparison(
    ...
    body = strikeouts_page(client)

    assert "vs MLB" in body
    assert "+1.00" in body
    assert "struck out 8.00 times per game" in body
    assert LEAGUE_STRIKEOUTS_UNAVAILABLE_NOTE not in body
...
def test_a_team_below_mlb_reads_as_a_negative_difference(
    ...
    seed_two_teams(seed)
    record_coverage(teams=2)
    assert "-1.00" in strikeouts_page(client, team_id=TWINS_ID)
...
def test_the_page_keeps_the_existing_series_alongside_the_mlb_line(
    ...
    seed_two_teams(seed)
    record_coverage(teams=2)
    body = strikeouts_page(client, window=10)
...
def test_the_page_reads_the_difference_as_descriptive_context(
    ...
    """More batting strikeouts than MLB is neither good nor bad on this page."""
    seed_two_teams(seed)
    record_coverage(teams=2)
    body = strikeouts_page(client)
    assert "struck out more times per game" in body
    assert "not a measure of significance" in body
    assert "neither direction is better by itself" in body


def test_the_page_still_says_k_per_game_is_not_k_rate(
    client: TestClient, seed: SeedFn, record_coverage: CoverageFn
) -> None:
    """MLB context does not turn a per-game count into an opportunity rate."""
    seed_two_teams(seed)
    record_coverage(teams=2)
    body = strikeouts_page(client)
...
def test_unequal_game_counts_are_weighted_on_the_page(
    ...
    """Mariners 10 K over 20 games, Twins 4 K in 1: MLB is 204/21, not 7.00.
    ...
    body = strikeouts_page(client)
    assert "struck out 9.71 times per game" in body
    assert "+0.29" in body
...
def test_incomplete_coverage_withholds_the_mlb_average(
    ...
    assert LEAGUE_STRIKEOUTS_UNAVAILABLE_NOTE in body
...
def test_incomplete_coverage_still_renders_the_team_chart(
    ...
    assert "team-strikeouts-chart" in body
    assert "Game Strikeouts" in body
...
def test_coverage_for_another_season_does_not_unlock_this_one(
    ...
    assert LEAGUE_STRIKEOUTS_UNAVAILABLE_NOTE in strikeouts_page(client, season=2026)
    assert LEAGUE_STRIKEOUTS_UNAVAILABLE_NOTE not in strikeouts_page(
        client, season=2025
    )
...
def test_complete_coverage_of_a_partial_season_still_compares(
    ...
    body = strikeouts_page(client, season=2026)
    assert "MLB Average" in body
    assert "struck out 8.00 times per game" in body
    assert "+2.00" in body
...
def test_legacy_null_league_rows_withhold_the_mlb_average(
    ...
    """Another club's rows predate strikeouts being stored: no MLB average.
...
def test_backfilling_the_league_makes_the_comparison_appear(
    ...
    seed([None] * 20, team_id=TWINS_ID, team_name=TWINS_NAME)
    record_coverage(teams=2)
    assert "MLB Average" not in strikeouts_page(client)
...
def test_the_team_and_season_selectors_still_work(
    ...
    assert "Minnesota Twins — Batting Strikeouts per Game" in strikeouts_page(
        client, team_id=TWINS_ID
    )
    assert "2026 regular season" in strikeouts_page(client, season=2026)
...
def test_the_window_selector_still_works_with_mlb_context(
    ...
    seed_two_teams(seed)
    record_coverage(teams=2)
    body = strikeouts_page(client, window=window)
...
def test_shareable_query_parameters_still_round_trip(
    ...
    seed_two_teams(seed)
    record_coverage(teams=2)
    body = strikeouts_page(client, team_id=TWINS_ID, window=30)
    assert "Minnesota Twins — Batting Strikeouts per Game" in body
    assert 'href="/?team_id=142&amp;season=2025&amp;window=30"' in body


def test_the_hits_page_is_unchanged_by_the_strikeout_comparison(
    client: TestClient, seed: SeedFn, record_coverage: CoverageFn
) -> None:
    seed_two_teams(seed)
    record_coverage(teams=2)
    ...
    assert "Batting Strikeouts per Game" not in body
...
def test_the_comparison_never_reaches_the_mlb_api(
    ...
    assert "MLB Average" in strikeouts_page(client)
...
def test_the_backfill_state_does_not_try_to_import(
    ...
    assert "import_league_season.py" in strikeouts_page(client)
